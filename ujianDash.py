...
app.layout = html.Div([
        ...
        html.P('Created by: Michael Candy'),
        ...
        dcc.Tabs(value = 'tabs', id = 'tabs-1', children = [
            ...
            dcc.Tab(label = 'Bar-Chart', value = 'tab-satu', children=
             [html.Div(children =[
                html.Div(children =[
                html.P('Y1:'),    
                dcc.Dropdown(id = 'y-axis-1', options = [{'label': i, 'value': i} for i in tsa.select_dtypes('number').columns], 
                value = 'Claim Amount')
                ], className = 'col-3'),
                html.Div(children =[
                html.P('Y2:'),    
                dcc.Dropdown(id = 'y-axis-2', options = [{'label': i, 'value': i} for i in tsa.select_dtypes('number').columns], 
                value = 'Close Amount')
                ], className = 'col-3'),
                html.Div(children =[
                html.P('X:'),    
                ...
                figure ={
                    'data' : [
                        ...
                        {'x': tsa['Claim Type'], 'y': tsa['Close Amount'], 'type': 'bar', 'name': 'Close Amount'}
                    ], 
            ...
            dcc.Tab(label ='Pie-Chart', value = 'tab-tiga', children =[
                html.Div(dcc.Dropdown(id ='pie-dropdown', options = [{'label': i, 'value': i} for i in tsa.select_dtypes('number').columns], value = 'Claim Amount'), 
                className = 'col-3'),
                html.Div([
                ## Graph Pie
                dcc.Graph(
                ...
                figure ={
                    'data' : [
                        go.Pie(labels = ['{}'.format(i) for i in list(tsa['Claim Type'].unique())], 
                                        values = [tsa.groupby('Claim Type').mean()['Claim Amount'][i] for i in list(tsa['Claim Type'].unique())],
                                        sort = False)
                    ], 
...
def create_graph_bar(y1, y2, x1):
    figure = {
                    'data' : [
                        {'x': tsa[x1], 'y': tsa[y1], 'type': 'bar', 'name' :y1},
                        {'x': tsa[x1], 'y': tsa[y2], 'type': 'bar', 'name': y2}
                    ], 
                    'layout': {'title': 'Bar Chart'}  
                    }
    return figure                
...
def create_graph_pie(x):
    figure = {
                    'data' : [
                        go.Pie(labels = ['{}'.format(i) for i in list(tsa['Claim Type'].unique())], 
                                        values = [tsa.groupby('Claim Type').mean()[x][i] for i in list(tsa['Claim Type'].unique())],
                                        sort = False)
                    ], 
                        'layout': {'title': 'Mean Pie Chart'}}
